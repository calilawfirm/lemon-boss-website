...
// About page — from calilawfirm.com
...
// Attorneys page — profile adapted from calilawfirm.com/about
function AttorneysPage({ go, onEstimator }) {
  return (
    <div>
      ...
      <section style={{ background: '#FFFDF5', padding: '72px 0' }}>
        <div className="container-narrow">
          <div style={{ background: '#fff', border: '3px solid #000', borderRadius: 28, boxShadow: '8px 8px 0 #000', padding: 32, display: 'grid', gridTemplateColumns: '220px 1fr', gap: 32, alignItems: 'start' }} className="attorney-card">
...
function ContactPage({ go }) {
  ...
  const input = (inv) => ({ width: '100%', padding: '13px 15px', border: `2px solid ${inv ? '#E53935' : '#000'}`, borderRadius: 12, background: '#fff', fontSize: 15, boxSizing: 'border-box', fontFamily: "'Inter', system-ui, sans-serif" });
  const label = { display: 'block', fontSize: 11, fontWeight: 800, letterSpacing: '.08em', textTransform: 'uppercase', color: '#262622', marginBottom: 6 };
            ...
            {sent ? (
              ...
              <form onSubmit={submit} style={{ display: 'grid', gap: 14 }}>
                ...
                <div>
                  ...
                  <input style={input(touched.name && !valid.name)} value={form.name} onChange={e => set('name', e.target.value)} onBlur={() => mark('name')} placeholder="Maya Rodriguez"/>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div>
                    ...
                    <input style={input(touched.email && !valid.email)} value={form.email} onChange={e => set('email', e.target.value)} onBlur={() => mark('email')} placeholder="you@example.com"/>
                  ...
                  <div>
                    ...
                    <input style={input(false)} value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="(555) 555-5555"/>
                  </div>
                </div>
                <div>
                  <label style={label}>What is this about?</label>
                  <select style={input(false)} value={form.topic} onChange={e => set('topic', e.target.value)}>
                ...
                <div>
                  ...
                  <textarea style={{ ...input(touched.message && !valid.message), resize: 'none' }} rows={5} value={form.message} onChange={e => set('message', e.target.value)} onBlur={() => mark('message')} placeholder="A quick summary of what's going on."/>
    ...
    <div style={{ background: '#FFFDF5', padding: '80px 0', minHeight: 'calc(100vh - 300px)' }}>
      <div className="container-narrow" style={{ maxWidth: 480 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <img src="assets/logo-lemon-boss.png" style={{ height: 56 }}/>
        </div>
        <div style={{ background: '#fff', border: '3px solid #000', borderRadius: 28, boxShadow: '8px 8px 0 #000', padding: 32 }}>
